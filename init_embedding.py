...
class JSSPInitEmbedding(nn.Module):
    ...
    def features(self,td:TensorDict):
        ...
        position_in_job = td['pos_in_job']#(bs,operations)  # WARNING: shape mismatch i think
...
class JsspEdgeEmbedding(nn.Module):
    '''Edge embeddings for static JSSP instance
        no embeddings for edge attributes
    '''
    def __init__(self, embed_dim, self_loop=False, **kwargs):
        ...
        super(JsspEdgeEmbedding,self).__init__()
    ...
    def _make_edge_attributes(self, proc_times:Tensor,num_machines:int):
        ...
        # job precedence edge indexes
        num_ops = proc_times.shape[1]
        num_jobs = num_ops // num_machines
        op_ids = torch.arange(num_ops, device=proc_times.device)
        op_ids = op_ids.view(num_jobs, num_machines) # view here is better cause .arange is contiguous...probably
        src = op_ids[:,:-1]
        dst = op_ids[:,1:]
        job_edge_index = torch.stack([src.reshape(-1),dst.reshape(-1)],dim=0)
        
        #shitty ai solution
        #machine sharing edge indices
        ...
        if len(machine_edges) > 0:
            ...
            print('warning: machine edges are 0') #change to logger
            machine_edge_index = torch.empty((2,0), dtype=torch.long, device=proc_times.device)

        edge_index = torch.cat([job_edge_index,machine_edge_index],dim=1)
        edge_attr = None
        ...
        for index, proc_times in enumerate(batch_proc_times): 
            ...
            graph = Data(
                x=init_embeddings[index],
                  ...
                  edge_attr=torch.zeros((m, self.embed_dim), device=device), # by passed the edge attr and gave zero embeddings 
            )
            graph_data.append(graph)
        
        batch = Batch.from_data_list(graph_data)
        # batch.edge_attr = self.edge_embed(batch.edge_attr)       useless rn (zero embedding)
        return batch
